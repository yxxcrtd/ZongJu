...
/*Editorial.CRM.CorpTypePropClassify.role = function(id) {
...
Editorial.CRM.CorpTypePropClassify.editObj = function(id) {
	...
	var url = $('#ctx').val() + "/pages/crm/corpTypePropClassify/form/edit?id="+id+"&tid="+$('#tid').val() ;
	var commonModalCss = {
		"width" : "540px",
		"height" : "330px"
	};
...
Editorial.CRM.CorpTypePropClassify.delObj = function(id) {
	openConfirmModalInFrame("您确定执行删除属性分类操作吗？",function() {
		var url = $('#ctx').val() + "/pages/crm/corpTypePropClassify/form/delete?id=" + id;
		$.ajax({
			"dataType" : 'json',
			"type" : "POST",
			"url" : url,
			...
			.dataTable(
					{
						...
						"sAjaxSource" : $('#ctx').val()+"/pages/crm/corpTypePropClassify/form/manager?tid="+$('#tid').val(), // mvc后台ajax调用接口。
