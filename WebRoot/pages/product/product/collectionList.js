...
Editorial.Product.ProductInfo.Parent.saveAndClose = function () {
    ...
    $.ajax({
        "dataType": 'json',
        "type": "POST",
        "url": $('#ctx').val() + "/pages/productCollection/form/saveParent?parentId=" + id+"&productId="+$('#productId').val(),
        "cache": false,
...
$(function () {
    ...
    Editorial.Product.ProductInfo.Parent.oTable1 = $('#table_report').dataTable({
        ...
        "sAjaxSource": $('#ctx').val() + "/pages/product/form/manager?flag=product&productType=collection", // mvc后台ajax调用接口。
