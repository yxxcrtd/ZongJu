...
BMEP.ProductType.classify = function(id) {
	var url = $('#ctx').val()+"/pages/pTypePropClassify/form/index?tid="+id;
	window.location.href = url;
};

BMEP.ProductType.productTypeProp = function(id) {
	var url = $('#ctx').val()+"/pages/productTypeProp/form/index?tid="+id;
	window.location.href = url;
};
...
$(function() {
	BMEP.ProductType.oTable1 = $('#table_report').dataTable( {
            ...
            "bServerSide": true, //从服务器端取数据
           	"sAjaxSource": $('#ctx').val()+"/pages/productType/form/manager?now=" + new Date().getTime(), //mvc后台ajax调用接口。
            ...
            "aoColumns": [ { 
    			...
    			"mDataProp": "id"
                    ...
                    "fnRender": function (oObj) {
                        ...
                        var flag0 = '<button class="btn btn-mini btn-success" title="属性分类" data-toggle="tooltip" data-placement="bottom" onclick="BMEP.ProductType.classify(\'' + oObj.aData.id + '\')"><i class="icon-flag bigger-120"></i></button>';
                        var flag = '<button class="btn btn-mini btn-success" title="类型属性" data-toggle="tooltip" data-placement="bottom" onclick="BMEP.ProductType.productTypeProp(\'' + oObj.aData.id + '\')"><i class="icon-cloud bigger-120"></i></button>';
                        var end = '</div>';
