...
Editorial.User.addObj = function() {
	var url = $('#ctx').val()+"/pages/stock/ivwarehouseroom/form/edits";
	var commonModalCss = {
		"width": "450px",
...
Editorial.User.modObj = function(id,fs) {
	var url = $('#ctx').val()+"/pages/stock/ivwarehouseroom/form/edits?id="+id+"&fs="+fs;
	var commonModalCss = {
		"width": "450px",
		"margin": "100px 0 0 -450px"
	};
...
Editorial.User.delObj = function(id) {
	openConfirmModalInFrame(Global_Prompt_Delete_Message, function() {
		var url = $('#ctx').val()+"/pages/stock/ivwarehouseroom/form/delete?id="+id;
		$.ajax( {
			"dataType": 'json',
			"type": "POST",
			"url": url,
