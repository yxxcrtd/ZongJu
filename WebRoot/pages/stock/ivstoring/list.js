...
Editorial.User.addObj = function() {
	var url = $('#ctx').val()+"/pages/stock/ivstoring/form/edits";
	var commonModalCss = {
		"width": "450px",
...
Editorial.User.modObj = function(id,il,pi) {
	var url = $('#ctx').val()+"/pages/stock/ivstoring/form/edits?id="+id+"&il="+il+"&pi="+pi;
	var commonModalCss = {
		"width": "450px",
		"margin": "100px 0 0 -450px"
	};
...
Editorial.User.delObj = function(id) {
	openConfirmModalInFrame(Global_Prompt_Delete_Message, function() {
		var url = $('#ctx').val()+"/pages/stock/ivstoring/form/delete?id="+id;
		$.ajax( {
			"dataType": 'json',
			"type": "POST",
			"url": url,
