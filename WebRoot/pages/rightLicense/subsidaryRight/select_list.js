	...
	Editorial.SubsidaryRight.StructureDialog.oTable1 = $("#select_element_report").dataTable({
		...
		"aoColumns" : [{
			...
			"fnRender" : function(oObj) {
				...
				var sReturn = "";
				var title = null;
				if (oObj.aData.structure.structureType.name == "文字") {
					title = oObj.aData.structure.name;
				} else if (oObj.aData.structure.structureType.name == "图片") {
					title = oObj.aData.structure.keyword;
				}
				sReturn = '<label><input type="radio" name="ids" style="display: none;" value="' + oObj.aData.structure.id + ',' + title + '" /><span class="lbl"></span></label>';
				return sReturn;
			}
		}, {
			"sTitle" : "序号",
			...
			"sTitle" : "素材类型",
			"mDataProp" : "structure.structureType.name",
			"bSortable" : false
		}, {
			"sTitle" : "内容",
			"mData" : null,
