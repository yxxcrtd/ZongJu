...
$(function () {
    ...
    for (var k = 0; k < parseInt(flowNodeCount); k++) {
        ...
        var html = Editorial.Flow.processRepo.createDataTableHtml(k);

        dataTableHtmlArray[k] = html;

        $("#repoContent").append(dataTableHtmlArray[k]);

        Editorial.Flow.processRepo.createDataTable(k);
    }
    ;
...
Editorial.Flow.processRepo.addTask = function () {
    var url = $('#ctx').val() + "/pages/ftask/form/index?flag=2";
    var commonModalCss = {
        "width": "1200px",
        "height": "600px"
    };
...
Editorial.Flow.processRepo.accordionToggle = function (id, obj) {
...
Editorial.Flow.processRepo.createDataTableHtml = function (k, taskData) {
    var html = "";
    html = html + '<div class="table-header on" onclick="Editorial.Flow.processRepo.accordionToggle(\'task_' + k + '\', this);">';
    ...
    if ("text" == taskData.display) {
        ...
        html = html + '" onclick="Editorial.Flow.processRepo.addCondition(' + k + ');" value="添加条件"/>';
        html = html + '</span>';
        html = html + '</div>';
        html = html + '</div>';
    }
...
Editorial.Flow.processRepo.addCondition = function (k) {
...
Editorial.Flow.processRepo.changeCondition = function (k, row, value) {
...
Editorial.Flow.processRepo.createDataTable = function (k, display) {
    var columns = "";
    if ("select" == display) {
        columns = Editorial.Flow.processRepo.selectColumns;
    }
    if ("text" == display) {
        columns = Editorial.Flow.processRepo.textColumns;
    }
    ...
    tableArray[k] = $('#table_report' + k).dataTable({
        ...
        "fnServerData": Editorial.Flow.processRepo.retrieveCacheData,
...
Editorial.Flow.processRepo.selectColumns = [
    {
        "sTitle": "序号",
        "mDataProp": "id"
    },
...
Editorial.Flow.processRepo.textColumns = [
    {
        "sTitle": "序号",
        "mDataProp": "id"
    },
    {
        ...
        "fnRender": function (oObj) {
            ...
            if ("otherwise" == oObj.aData.conditionSymbol) {
                ...
                select = '<select id="conditionSymbol_select" name="conditionSymbol_select" class="span10" onblur="Editorial.Flow.processRepo.changeCondition(\'' + oObj.oSettings.sAjaxSource + '\', \'' + oObj.aData.id + '\', this.value)">';
...
Editorial.Flow.processRepo.retrieveCacheData = function (sSource, aoData, fnCallback) {
    //console.log("sSource:" + sSource);
    //console.log("aoData JSON:" + JSON.stringify(aoData));
    fnCallback(dataTableArray[sSource]);
};

Editorial.Flow.processRepo.selectField = function () {
...
Editorial.Flow.processRepo.updateField = function (jsonObj, context) {
    console.log(jsonObj);
    ...
    //alert(":" + $(context).html());
    //alert("taskName:" + $(context).repoContents().find("#taskName").val());
...
Editorial.Flow.processRepo.clearForm = function () {
    $("#taskName").val("");
    $("#taskUrl").val("");
...
Editorial.Flow.processRepo.addObj = function (jsonObj, context) {
    $.ajax({
        "dataType": 'json',
        "type": "POST",
        "url": $('#ctx').val() + "/pages/ftask/form/findTaskById?id=" + jsonObj,
        "cache": false,
...
//            console.log(response)
            ...
            var taskData = {
                ...
                taskAssigneeFlag: response.task.taskAssigneeFlag,
                assignee: response.task.assignee,
                startFlag: response.task.startFlag,
                //endFlag : $("#taskEndFlag").val(),
                field: response.task.field,
                display: response.task.display
            };
            console.log("1234" + taskData);
            // 将新添加的Task加入到taskMap中
            taskMap.put(response.task.name, taskData);
            console.log(response.dicMap);
            var taskFieldHidden = JSON.stringify(response.dicMap);
            var taskDisplayFieldHidden = response.task.display;
            ...
            // 创建DataTable Html代码
            var html = Editorial.Flow.processRepo.createDataTableHtml(index, taskData);
            ...
            for (var i = 0; i < dataTableHtmlArray.length; i++) {
                ...
                $("#repoContent").append(dataTableHtmlArray[i]);
                // 循环渲染所有DataTable的JS代码
                Editorial.Flow.processRepo.createDataTable(i, display);
            }


            Editorial.Flow.processRepo.clearForm();
        },
...
Editorial.Flow.processRepo.deploy = function () {
    ...
    $.ajax({
        ...
        "type": "POST",
        "url": $('#ctx').val() + "/pages/fprocess/form/saveProcess",
        "cache": false,
        "data": JSON.stringify({
            "taskList": jbpmTaskList,
        ...
        "success": function (response) {
            if (response.isSuccess == "true") {
                ...
                refreshFrameDataTableInFrame('Editorial.Flow.processRepo.list.oTable1');
...
Editorial.Flow.processRepo.selectTaskAssigneeFlag = function (value) {
    if (value != "") {
        if (value == "1") {
